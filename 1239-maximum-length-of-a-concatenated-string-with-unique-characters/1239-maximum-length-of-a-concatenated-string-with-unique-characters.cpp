...
public:
    ...
    bool isvalid(vector<bool> &ispicked, string &curr) {
        
        vector<bool> repeatingstring(26, false);
        
        for(int i=0; i<curr.size(); i++) {
            if(repeatingstring[curr[i] - 'a'])
                return false;
            repeatingstring[curr[i] - 'a'] = true;
        }
        
        for(int i=0; i<curr.size(); i++) {
            if(ispicked[curr[i] - 'a'])
                return false;
        }
